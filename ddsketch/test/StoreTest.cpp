...
TEMPLATE_TEST_CASE("test cross merge", "[storetest]",
  ...
  auto store = TestType::first_type::new_store();
  auto collapse = TestType::second_type::collapse;
  ...
  test_cross_merge(store, dense_store_1, collapse, collapse_1);
  ...
  test_cross_merge(store, dense_store_2, collapse, collapse_2);

  store->clear();
  auto dense_store_3 = store_factory<UnboundedSizeDenseStore<A>>::new_store();
  auto collapse_3 = noops_collapsing_bins::collapse;
  test_cross_merge(store, dense_store_3, collapse, collapse_3);

  store->clear();
  auto sparse_store = store_factory<SparseStore<A>>::new_store();
  auto collapse_sparse = noops_collapsing_bins::collapse;
  test_cross_merge(store, sparse_store, collapse, collapse_sparse);
}
